...
class Party(CreatedAtMixin, models.Model):
    ...
    def get_debt_by_member(self, user: User):
        ...
        sum_p = 0
        for pp in user.payments.filter(party_group=self):
            sum_p += pp.price
        # print(f'Потратил: {sum_p}')

        sum_d = 0
        for pd in user.debtors.filter(party_group=self):
            sum_d += pd.get_debt_value()
        # print(f'Должен: {sum_d}')

        sum_st = 0
        for pt in user.sender_transactions.filter(party_group=self):
            sum_st += pt.value
        # print(f'Отдал: {sum_st}')

        sum_rt = 0
        for pt in user.recipient_transactions.filter(party_group=self):
            sum_rt += pt.value
